...
export async function buildVentureExportMarkdown(project: { id: string; name: string }) {
    ...
    for (const key of VENTURE_ORDER) {
        ...
        const outputSafe = redactSecrets(stageSnapshot.latestContent);
    ...
    const doneApproved = VENTURE_ORDER.every((key) => snapshot.stages[key].approved);

    const slug = slugify(project.name) || project.id;
    const filename = `venture-fundamentos-${slug || project.id}.md`;

    return { markdown, filename, done, doneApproved };
}
