...
const TEST_PREFIX = "test_bp_stage2_fixed";
...
function assertSnapshot(actual: unknown) {
    ...
    if (process.env.UPDATE_BP_STAGE2_SNAPSHOT === "1") {
    ...
    if (!fs.existsSync(SNAPSHOT_PATH)) {
        throw new Error(
            "Stage 2 snapshot missing. Run: npm run test:business-plan:stage2:update-snapshot"
        );
    }
    ...
    if (expected.trim() !== actualSerialized.trim()) {
        throw new Error(
            [
                "Snapshot mismatch for Stage 2.",
                "Update scripts/tests/business-plan/__snapshots__/stage2.json if expected.",
                "--- Expected ---",
                expected.trim(),
                "--- Actual ---",
    ...
    try {
        ...
        const project = await prisma.project.create({
            data: {
                ...
                description: "Business plan engine stage 2 test",
                moduleVenture: true,
                status: "CREATED",
            },
        });
        ...
        const document = await businessPlanService.getDocument(
            businessPlanId,
            org.id
        );
        ...
        console.log("Business Plan Engine Stage 2 tests: OK");
    } finally {
        if (projectId) {
...
main().catch((error) => {
    console.error("Business Plan Engine Stage 2 tests failed:", error);
    process.exit(1);
});
