...
export function DashboardShell({ children, isSuperAdmin }: DashboardShellProps) {
    const [sidebarOpen, setSidebarOpen] = useState(false);
    const pathname = usePathname();
    const { organization, isLoaded, membership } = useOrganization();

    // Check if user is Org Admin (Owner or Admin)
    const isOrgAdmin = membership?.role === "org:admin" || membership?.role === "org:owner" || (membership as any)?.role === "OWNER" || (membership as any)?.role === "ADMIN";
    // Clerk uses "org:admin" format usually, but our DB sync might map to OWNER/ADMIN. 
    // Let's rely on Clerk's 'org:admin' check strictly or check both.
